...
// ---------- 真实音乐文件(免版税曲库,加载失败自动回退合成器) ----------
// 文件放 public/audio/,许可记录见 public/audio/CREDITS.md
const FILE_TRACKS = {
  gomoku: '/audio/gomoku.mp3',
  xiangqi: '/audio/xiangqi.mp3',
  junqi: '/audio/junqi.mp3',
  ludo: '/audio/ludo.mp3',
  doudizhu: '/audio/doudizhu.mp3',
  tank: '/audio/tank.mp3',
  snow: '/audio/snow.mp3',
  run: '/audio/run.mp3',
  fight: '/audio/fight.mp3',
};
const FILE_VOLUME = 0.22; // 真实音乐响度高,压低不盖语音
const fileFailed = new Set(); // 加载失败的 url(本次会话内不再重试)
...
/** 播放循环音乐文件;失败时调用 onFail 回退合成器 */
function playFile(url, onFail) {
  stopFile();
...
export function playScene(gameId, scene) {
  ...
  if (key === currentKey && (scene === 'playing' ? timer || audioEl : true)) return; // 幂等
  currentKey = key;
  pendingChain = null;
  clearTimer();
  loopTrack = null;

  if (scene === 'intro' || scene === 'playing') {
    // 真实音乐优先:有文件直接循环(跳过合成 intro);失败回退合成器
    const url = FILE_TRACKS[gameId];
    if (url && !fileFailed.has(url)) {
      currentKey = `${gameId}:playing`;
      playFile(url, () => {
        const t = trackOf(gameId, 'playing');
        if (t && isBgmEnabled()) startLoop(t);
      });
      return;
    }
  } else {
    stopFile(); // victory/defeat:停循环
  }
